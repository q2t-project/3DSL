// public/viewer/test/viewerRegressionRunner.js
//
// Viewer regression browser regression runner (manual but repeatable)
// Usage in console:
//   const { runAll } = await import("/viewer/test/viewerRegressionRunner.js?ts=" + Date.now());
//   await runAll(window.__viewerHub, { disposeAtEnd: false });

const UUID_CENTER = "11111111-1111-1111-1111-111111111111";

function log(line) {
  console.log(`[viewerRegressionRunner] ${line}`);
}
...
export async function runAll(hub, opts = {}) {
  ...
  if (!pick0.ok) {
    ...
    // Viewer regression の要件は「不可視は絶対に選べない」なので、uuid==center 固定はしない。
    if (hit == null) {
