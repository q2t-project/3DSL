...
export async function initViewerI18n(opts = {}) {
  ...
  // locales は viewer 配下に置く前提
  // 例:
  //   /viewer/locales/ja/translation.json
  //   /3dsl/viewer/locales/en/translation.json
  const loadPath = joinBase(viewerBase, "locales/{{lng}}/translation.json");

  const instance = i18next.createInstance();

  try {
    const lng = pickLng(opts);
    const url = loadPath.replace("{{lng}}", encodeURIComponent(lng));
    let resources = {};
    try {
