...
export async function createPublicUiBridge(opts = {}) {
  ...
  // - default path: `${base}viewer/locales/{lang}.json`
    ...
    try {
      const path =
        typeof opts?.localePath === "string" && opts.localePath.trim()
          ...
          : (base.endsWith("/") ? `${base}viewer/locales/` : `${base}/viewer/locales/`);

      // Try {lang}.json, then translation.json under {lang}/translation.json
      const url1 = `${path}${lang}.json`;
      const url2 = `${path}${lang}/translation.json`;

      const j1 = await _pubuiLoadLocaleJson(url1).catch(() => null);
      if (j1) _pubuiMergeLocale(lang, j1);
      else {
        const j2 = await _pubuiLoadLocaleJson(url2).catch(() => null);
        if (j2) _pubuiMergeLocale(lang, j2);
      }
    } catch (_e) {}
  }
      ...
      try {
        ...
        const path =
          typeof extra.localePath === "string" && extra.localePath.trim()
            ...
            : (base.endsWith("/") ? `${base}viewer/locales/` : `${base}/viewer/locales/`);
        const url = `${path}${lang2}.json`;
        const json = await _pubuiLoadLocaleJson(url);
