...
const fixturesDir = argValue("--dir", path.resolve(process.cwd(), "public/3dss/fixtures/phase7"));
...
let schemaPath =
  explicitSchema ||
  findExisting([
    path.resolve(process.cwd(), "public/3dss/3dss/release/3DSS.schema.json"),
    path.resolve(process.cwd(), "public/schemas/3DSS.schema.json"),
    path.resolve(process.cwd(), "public/schemas/3dss.schema.json"),
    path.resolve(process.cwd(), "schemas/3DSS.schema.json"),
    path.resolve(process.cwd(), "schemas/3dss.schema.json"),
    path.resolve(process.cwd(), "public/3dsl/schemas/3DSS.schema.json"),
    path.resolve(process.cwd(), "public/3dsl/schemas/3dss.schema.json"),
    path.resolve(process.cwd(), "public/viewer/schemas/3DSS.schema.json"),
    path.resolve(process.cwd(), "public/viewer/schemas/3dss.schema.json"),
  ]);
...
if (!schemaPath) {
  ...
  schemaPath =
    walkFindFile(path.resolve(process.cwd(), "public"), targets) ||
    walkFindFile(path.resolve(process.cwd(), "schemas"), targets) ||
    walkFindFile(process.cwd(), targets);
}
...
if (!schemaPath) {
  console.error("[phase7:validate] ERROR: schema not found.");
...
if (!fs.existsSync(fixturesDir)) {
  console.error(`[phase7:validate] ERROR: fixtures dir not found: ${fixturesDir}`);
  process.exit(1);
}

console.log(`[phase7:validate] schema = ${path.relative(process.cwd(), schemaPath)}`);
...
try {
  ...
  console.error("[phase7:validate] ERROR: failed to compile schema (Ajv).");
...
if (files.length === 0) {
  console.error(`[phase7:validate] ERROR: no fixtures found in ${fixturesDir}`);
  process.exit(1);
}
...
for (const fp of files) {
  ...
  try {
    ...
    console.error(`[phase7:validate] NG JSON parse: ${fp}`);
  ...
  if (!expectInvalid) {
    if (!hasErrors) {
      console.log(`[phase7:validate] OK  ${rel}`);
      okValid++;
    } else {
      console.log(`[phase7:validate] NG  ${rel}`);
      if (!schemaOk) {
    ...
    // invalid は「落ちたらOK」
    if (hasErrors) {
      console.log(`[phase7:validate] OK(invalid)  ${rel}`);
      okInvalid++;
    } else {
      console.log(`[phase7:validate] NG(unexpected valid)  ${rel}`);
      ngCount++;
    }
  }
}

console.log(`[phase7:validate] summary valid_ok=${okValid} invalid_ok=${okInvalid} ng=${ngCount}`);
