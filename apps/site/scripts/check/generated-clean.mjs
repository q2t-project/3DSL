...
// NOTE: When the repo is provided as a zip (no .git directory), we can't
// reliably tell what is "tracked" vs "generated". In that environment, we
// only enforce the filesystem-only guards below (no /scripts, no *.mjs), and
// skip the git-tracked validation.
const hasGit = fs.existsSync(path.join(repoRoot, ".git"));

// In zip-only sandboxes there is no .git, so we can't verify "tracked" files.
// We still run filesystem-only guards below (no /scripts, no *.mjs).
const GENERATED_DIRS = [
  ...
  // Generated by sync:3dss-content
...
const ALLOWLIST = new Set([
  ...
  "apps/site/public/modeler_app/.gitkeep",
  "apps/site/public/vendor/.gitkeep",
  "apps/site/public/schemas/.gitkeep",

  "apps/site/public/_data/.gitkeep",
  "apps/site/public/fixtures/.gitkeep",
...
// Guard: ensure ignore rules exist so generated mirrors cannot be accidentally staged.
// This complements the git-index scan below. In practice, most accidents happen because
// ignore rules are missing/removed, then `git add -A` sweeps in huge mirrors.
const REQUIRED_IGNORE_PATTERNS = [
  // Generated mirrors under apps/site/public
  "apps/site/public/viewer/",
  "apps/site/public/modeler_app/",
  "apps/site/public/vendor/",
  "apps/site/public/schemas/",
  ...
  // Docs mirrors (SSOT lives in packages/docs/*)
  "apps/site/src/content/docs/",
  "apps/site/src/content/faq/",
  "apps/site/src/content/policy/",
];

function verifyIgnoreRules() {
  const ignorePath = path.join(repoRoot, "apps/site/.gitignore");
  if (!fs.existsSync(ignorePath)) {
    throw new Error(`[generated-clean] Missing ${ignorePath}.`);
  }
  const text = fs.readFileSync(ignorePath, "utf8");
  const missing = REQUIRED_IGNORE_PATTERNS.filter((p) => !text.includes(p));
  if (missing.length) {
    console.error("[generated-clean] NG: required ignore patterns are missing in apps/site/.gitignore:\n");
    for (const p of missing) console.error(`  - ${p}`);
    console.error("\nFix: add the above lines to apps/site/.gitignore (generated mirrors must not be tracked).");
    process.exit(1);
  }
}
...
<<<<<<< HEAD
// Run ignore rules check first so the failure message is precise and actionable.
verifyIgnoreRules();
...
if (bad.length > 0) {
  ...
  console.error("  git rm -r --cached apps/site/public/viewer apps/site/public/modeler_app apps/site/public/vendor apps/site/public/schemas apps/site/public/_data apps/site/public/fixtures");
...
=======
  ...
  console.warn("[generated-clean] NOTE: .git not found; skipping tracked-file validation (zip/sandbox context).");
>>>>>>> origin/main
}
