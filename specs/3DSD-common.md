...
Frame は 一次元整数 ID。
multi-axis frame / multi-dimensional frame はスキーマの対象外。

Viewer / Modeler は同一の activeFrameId の意味を共有する。

UI 設計において、frame の連続性を示す手段（視覚的ラチェット感）は
各アプリの責務であり、スキーマは関知しない。

Frame 0 を基準とし、表示の起点・軸とする。
