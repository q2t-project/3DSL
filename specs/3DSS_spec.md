...
すべての nodes / edges / text / aux / gltf はこの規範に基づいて解釈される。

### 座標系
- 基底ベクトルは (X, Y, Z) とし、**X×Y = Z が成り立つ直交座標系**とする。  
- 原点 (0,0,0) は**ローカルシーンの中心**を表す。  
- 単位系は**無次元**とする。  
...
### 回転規範
- 平面における `rotation` の指定は、各平面の**法線軸まわりに CW（時計回り）を正**とする。  
