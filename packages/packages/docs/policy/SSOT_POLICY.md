...
## 6.5 Repo root は npm-free（運用固定）

この repo は **root を npm 管理から外す**。

- root に `package.json` / `package-lock.json` / `npm-shrinkwrap.json` / `node_modules` を置かない
- **root で npm を実行しない**（勝手に workspace / 親パッケージ扱いになって破壊的）
- npm は **各アプリ直下**だけで実行する：
  - `apps/site/`
  - `apps/viewer/`（npm を使う場合のみ）
  - `apps/modeler/`（npm を使う場合のみ）

機械ガード：
- `scripts/check/no-root-npm.ps1`（Windows / PowerShell）
- `scripts/check/no-root-npm.sh`（CI / bash）
