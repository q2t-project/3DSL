````markdown
# 3DSS (Three-Dimensional Structural Schema) 仕様書 v1.1.1
...
| スキーマURI | https://q2t-project.github.io/3dsl/schemas/3DSS.schema.json#v1.1.1 |
...
スキーマの `$id` は `https://q2t-project.github.io/3dsl/schemas/3DSS.schema.json#` とし、バージョンは `$anchor`（例：`v1.1.1`）で表す。構造ドキュメント側は `document_meta.schema_uri` に `...json#vX.Y.Z` を格納して、準拠バージョンを明示する。
構造ドキュメントのバージョン管理は `document_meta.version`（SemVer）で行い、
仕様書の版数（本書 v1.1.1）とは区別する。
...
- `schema_uri` には、原則として本スキーマの `$id` に `$anchor`（SemVer）を付けた値（例：`https://q2t-project.github.io/3dsl/schemas/3DSS.schema.json#v1.1.1`）を格納する。  
...
```
第3章と第4章を v1.1.0 の実スキーマに揃えた版をまとめる。
（`$defs` の構成やプロパティ名は、いま貼ってくれた 3DSS.schema.json v1.1.0 構造に合わせてある）
...
3DSS v1.1.0 では、スキーマ設計において **論理的一貫性と拡張可能性** を優先し、表現上の自由度はその範囲で確保する。
...
v1.1.0 における `$defs` の主な構成は次の通りである。
...
v1.1.0 における `$defs` の主要な構成要素と役割を整理する。
...
---
ほな、第5章と第6章を v1.1.0 のスキーマに揃えた版をまとめて出すで。
（見出し構成はそのまま、内容だけ 3DSS.schema.json v1.1.0 にフィットさせてある）
...
`required_sets` や `enum_constraint` といった共通制約は v1.1.1 時点では `$defs` に未定義であり、
...
| `caption_text`    | object                   | 任意     | –                 | キャプション表示の見た目設定。`font_size` と `plane`。 |
...
`caption_text` は line の `signification.caption` を表示するためのスタイル情報（v1.1.1）。

| プロパティ | 型 | 必須 | デフォルト | 説明 |
|---|---|---|---|---|
| `font_size` | number | 任意 | `8` | 表示サイズ（相対）。Viewer は `font_size/8` を倍率として適用する想定。 |
| `plane` | string(enum) | 任意 | `"billboard"` | 表示平面。`xy` / `yz` / `zx` / `billboard`。v0 は `billboard` のみ対応で良い。 |
...
* line 間の直接参照（line → line）は v1.1.1 では未サポート。将来拡張領域とする。
* 自己参照や循環参照、別ファイルへの越境参照の禁止／許可は、アプリケーション側の検証ポリシで扱う。

JSON Schema v1.1.1 では、UUID 形式のみを検証対象とし、
...
| `schema_uri`        | string(uri)           | ✅  | –                                       | 準拠スキーマ URI。例：`https://q2t-project.github.io/3dsl/schemas/3DSS.schema.json#v1.1.1` |
...
v1.1.0 では、以下のキーを持つ（実体はすべて `$defs.validator.*` として定義される）。
...
v1.1.0 では `parametric` と `latex` の 2 種を定義する。
...
| `plane`       | string(enum)        | 任意 | –         | 貼付面。`xy` / `yz` / `zx` / `billboard`。     |
| `position`    | vec3                | 任意 | –         | 配置位置。                                     |

* `plane` / `position` にも JSON Schema 上の default は定義されていない。
  Viewer 実装は、未指定時に例えば plane=`"zx"`・position=`[0,0,0]` を初期値とするなど、
  アプリ側のポリシーで補完してよい。
...
3DSS v1.1.0 では座標系は `document_meta.coordinate_system` の const により
**`"Z+up/freeXY"` に固定**され、
...
* 新要素や列挙値（enum）の追加は、該当プロパティ定義（第 5 章）に対して直接 `enum` を拡張し、
  ...
  v1.1.0 時点でも `$defs.validator.enum_constraint` は定義しない。
...
* **スキーマ URI ポリシー**：
  ...
  * 例：`3DSS.schema.v1.0.3.json`・`3DSS.schema.v1.1.0.json` などの形で履歴管理してもよい（命名規則は運用に委ねる）。
...
* ✅ `$defs` 拡張ルールと外部スキーマ参照方針を v1.1.0 構造に合わせて整理。
...
* `schema_uri` の値を `$id` と同一の `https://q2t-project.github.io/3dsl/schemas/3DSS.schema.json#v1.1.1`
 に揃えている（バージョン別フラグメントは付与しない方針）。
* `aux.module.grid/axis` の構造

など、v1.1.0 で整理した定義と揃えてある。
