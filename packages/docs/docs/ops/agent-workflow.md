# Agent workflow (migrated from legacy AGENTS.md)

この文書は、過去に `AGENTS.md` に混在していた **作業規律・ガイドライン**を、入口ファイルの肥大化と
`AGENTS*.md` の乱立を避けるために **移設**したもの。
...
以下は移設元の内容を **そのまま** 収録している（必要に応じて将来、章立て整理は別途行う）。
