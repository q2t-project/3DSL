...
Public Sub Export3DSSJson(Optional ByVal jsonPath As String = "")
  ...
  doc("document_meta") = ReadDocumentMeta()
  doc("points") = ReadPoints()
  doc("lines") = ReadLines()
  doc("aux") = ReadAux()
...
Private Function ReadPoints() As Collection
  ...
  For r = FIRST_DATA_ROW To lastRow
    ...
    ApplyEditsByMapping ws, cols, r, node, map, touched
...
Private Sub ApplyEditsByMapping(ByVal ws As Worksheet, ByVal cols As Object, ByVal r As Long, ByVal node As Object, ByVal map As Collection, ByVal touched As Object)
  ...
  For i = 1 To map.Count
    ...
    If h = "frames" Or uiKey = "frames_json" Then
      value = ParseFramesCell(cellS)
    ElseIf h = "json" Or vt = "json" Or vt = "tags_json" Then
      value = CellToJsonValue(cellS)
    Else
      value = CellToTypedValue(cellS, vType)
    End If
...
Private Function MakeEmptyPoint(ByVal uuid As String) As Object
  ...
  n("meta") = meta
  n("signification") = CreateDict()
  n("appearance") = CreateDict()
...
Private Sub ClearDataRows(ByVal ws As Worksheet, ByVal clearCols As Long)
  ...
  ws.Range(ws.Cells(FIRST_DATA_ROW, 1), ws.Cells(lastRow, clearCols)).ClearContents
End Sub
...
Private Sub JsonSetByPath(ByVal root As Object, ByVal path As String, ByVal value As Variant)
  ...
  For i = LBound(parts) To UBound(parts)
    ...
    If idx >= 0 Then
      ' intermediate array: ensure collection and descend into object element
      Dim coll2 As Collection
      If IsObject(cur(name)) And TypeName(cur(name)) = "Collection" Then
        Set coll2 = cur(name)
      Else
      ...
      Dim child As Object
      If coll2.Count >= idx + 1 And IsObject(coll2(idx + 1)) Then
