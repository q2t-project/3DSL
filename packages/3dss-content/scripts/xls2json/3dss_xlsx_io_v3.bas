...
'
' Goals:
' - XLSX は自由設計（人が編集しやすい列名）。
' - VBA 側で「列キー -> JSON 構造」を明示マッピング。
' - 列の並び替えに強い（ヘッダ行1のキーで列位置を解決）。
' - JSON配列/オブジェクトは *_json 列に「JSON文字列」として格納。
'
' Sheets:
...
NextR:
  ...
  If (Not HasKey(m, "schema_uri")) Or Len(Trim$(CStr(m("schema_uri")))) = 0 Then m("schema_uri") = "（ここは運用に合わせて固定）"
...
Private Sub WritePointRow(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal pt As Variant)
  ...
  If Not meta Is Nothing Then
    Put ws, col, r, "uuid", GetAny(meta, "uuid")
    If HasKey(meta, "tags") Then Put ws, col, r, "tags_json", ConvertToJson(meta("tags"), 0)
    Put ws, col, r, "creator_memo", GetAny(meta, "creator_memo")
  End If
  ...
  If Not app Is Nothing Then
    ...
    Put ws, col, r, "x", ArrGet(pos, 0)
    Put ws, col, r, "y", ArrGet(pos, 1)
    Put ws, col, r, "z", ArrGet(pos, 2)

    Put ws, col, r, "visible", GetAny(app, "visible")

    Dim fr As Variant: fr = GetAny(app, "frames")
    If Not IsEmpty(fr) Then
      If IsObject(fr) Or IsArray(fr) Then
        Put ws, col, r, "frames", ConvertToJson(fr, 0)
      Else
        Put ws, col, r, "frames", fr
      End If
    End If

    Dim marker As Object: Set marker = GetObj(app, "marker")
    If Not marker Is Nothing Then
      Put ws, col, r, "marker_primitive", GetAny(marker, "primitive")
      Put ws, col, r, "marker_radius", GetAny(marker, "radius")
      Put ws, col, r, "marker_height", GetAny(marker, "height")

      Dim sizeV As Variant: sizeV = GetAny(marker, "size")
      Put ws, col, r, "marker_size_x", ArrGet(sizeV, 0)
      Put ws, col, r, "marker_size_y", ArrGet(sizeV, 1)
      Put ws, col, r, "marker_size_z", ArrGet(sizeV, 2)

      Dim baseV As Variant: baseV = GetAny(marker, "base")
      Put ws, col, r, "marker_base_x", ArrGet(baseV, 0)
      Put ws, col, r, "marker_base_y", ArrGet(baseV, 1)

      Put ws, col, r, "corona_inner_radius", GetAny(marker, "inner_radius")
      Put ws, col, r, "corona_outer_radius", GetAny(marker, "outer_radius")

      Dim gltf As Object: Set gltf = GetObj(marker, "gltf")
      If Not gltf Is Nothing Then
        Put ws, col, r, "gltf_url", GetAny(gltf, "url")
        Dim sc As Variant: sc = GetAny(gltf, "scale")
        Put ws, col, r, "gltf_scale_x", ArrGet(sc, 0)
        Put ws, col, r, "gltf_scale_y", ArrGet(sc, 1)
        Put ws, col, r, "gltf_scale_z", ArrGet(sc, 2)

        Dim rot As Variant: rot = GetAny(gltf, "rotation")
        Put ws, col, r, "gltf_rot_x", ArrGet(rot, 0)
        Put ws, col, r, "gltf_rot_y", ArrGet(rot, 1)
        Put ws, col, r, "gltf_rot_z", ArrGet(rot, 2)

        Dim off As Variant: off = GetAny(gltf, "offset")
        Put ws, col, r, "gltf_off_x", ArrGet(off, 0)
        Put ws, col, r, "gltf_off_y", ArrGet(off, 1)
        Put ws, col, r, "gltf_off_z", ArrGet(off, 2)
      End If

      Dim txt As Object: Set txt = GetObj(marker, "text")
      If Not txt Is Nothing Then
        Put ws, col, r, "text_content", GetAny(txt, "content")
        Put ws, col, r, "text_font", GetAny(txt, "font")
        Put ws, col, r, "text_size", GetAny(txt, "size")
        Put ws, col, r, "text_align", GetAny(txt, "align")
        Put ws, col, r, "text_plane", GetAny(txt, "plane")
      End If
      ...
      If Not common Is Nothing Then
        ...
        Put ws, col, r, "common_orient_x", ArrGet(ori, 0)
        Put ws, col, r, "common_orient_y", ArrGet(ori, 1)
        Put ws, col, r, "common_orient_z", ArrGet(ori, 2)

        Dim sc2 As Variant: sc2 = GetAny(common, "scale")
        Put ws, col, r, "common_scale_x", ArrGet(sc2, 0)
        Put ws, col, r, "common_scale_y", ArrGet(sc2, 1)
        Put ws, col, r, "common_scale_z", ArrGet(sc2, 2)

        Put ws, col, r, "common_color", GetAny(common, "color")
        Put ws, col, r, "common_opacity", GetAny(common, "opacity")
        Put ws, col, r, "common_emissive", GetAny(common, "emissive")
        Put ws, col, r, "common_wireframe", GetAny(common, "wireframe")
      End If
    End If
  End If
End Sub
...
Private Function ReadPoints() As Collection
  ...
  For r = DATA_START_ROW To lastRow
    ...
    If IsBlank(xV) Or IsBlank(yV) Or IsBlank(zV) Then
      Err.Raise 10001, "ReadPoints", "points row " & r & ": x/y/z は必須"
    End If
...
Private Function BuildPointMarker(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long) As Object
  ...
  Select Case prim
    Case "sphere"
      If IsBlank(GetCell(ws, col, r, "marker_radius")) Then Err.Raise 10001, "BuildPointMarker", "points row " & r & ": sphere は marker_radius 必須"
    ...
    Case "box"
      If IsBlank(GetCell(ws, col, r, "marker_size_x")) Or IsBlank(GetCell(ws, col, r, "marker_size_y")) Or IsBlank(GetCell(ws, col, r, "marker_size_z")) Then
        Err.Raise 10001, "BuildPointMarker", "points row " & r & ": box は marker_size_x/y/z 必須"
      End If
    ...
    Case "cone"
      If IsBlank(GetCell(ws, col, r, "marker_radius")) Or IsBlank(GetCell(ws, col, r, "marker_height")) Then
        Err.Raise 10001, "BuildPointMarker", "points row " & r & ": cone は marker_radius + marker_height 必須"
      End If
      m("radius") = CDbl(GetCell(ws, col, r, "marker_radius"))
    ...
    Case "pyramid"
      If IsBlank(GetCell(ws, col, r, "marker_base_x")) Or IsBlank(GetCell(ws, col, r, "marker_base_y")) Or IsBlank(GetCell(ws, col, r, "marker_height")) Then
        Err.Raise 10001, "BuildPointMarker", "points row " & r & ": pyramid は marker_base_x/y + marker_height 必須"
      End If
    ...
    Case "corona"
      If IsBlank(GetCell(ws, col, r, "corona_inner_radius")) Or IsBlank(GetCell(ws, col, r, "corona_outer_radius")) Then
        Err.Raise 10001, "BuildPointMarker", "points row " & r & ": corona は corona_inner_radius + corona_outer_radius 必須"
      End If
...
Private Sub WriteLineRow(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal ln As Variant)
  ...
  If Not sig Is Nothing Then
    ...
    If IsObject(rel) Then
      ...
      For Each kk In rd.Keys
        Put ws, col, r, "relation_kind", kk
        Put ws, col, r, "relation_value", rd(kk)
        Exit For
      Next kk
    End If

    Put ws, col, r, "sense", GetAny(sig, "sense")
  ...
  If Not app Is Nothing Then
    ...
    If Not ea Is Nothing Then
      Put ws, col, r, "end_a_ref", GetAny(ea, "ref")
      Dim coord As Variant: coord = GetAny(ea, "coord")
      Put ws, col, r, "end_a_x", ArrGet(coord, 0)
      Put ws, col, r, "end_a_y", ArrGet(coord, 1)
      Put ws, col, r, "end_a_z", ArrGet(coord, 2)
    End If

    Dim eb As Object: Set eb = GetObj(app, "end_b")
    If Not eb Is Nothing Then
      Put ws, col, r, "end_b_ref", GetAny(eb, "ref")
      Dim coord2 As Variant: coord2 = GetAny(eb, "coord")
      Put ws, col, r, "end_b_x", ArrGet(coord2, 0)
      Put ws, col, r, "end_b_y", ArrGet(coord2, 1)
      Put ws, col, r, "end_b_z", ArrGet(coord2, 2)
    End If

    Put ws, col, r, "line_type", GetAny(app, "line_type")
    Put ws, col, r, "line_style", GetAny(app, "line_style")
    Put ws, col, r, "color", GetAny(app, "color")
    Put ws, col, r, "opacity", GetAny(app, "opacity")
    Put ws, col, r, "render_order", GetAny(app, "render_order")
    Put ws, col, r, "visible", GetAny(app, "visible")
    ...
    If Not IsEmpty(geo) Then Put ws, col, r, "geometry_json", ConvertToJson(geo, 0)

    Dim arrow As Object: Set arrow = GetObj(app, "arrow")
    If Not arrow Is Nothing Then
      Put ws, col, r, "arrow_primitive", GetAny(arrow, "primitive")
      Put ws, col, r, "arrow_placement", GetAny(arrow, "placement")
      Put ws, col, r, "arrow_auto_orient", GetAny(arrow, "auto_orient")
      Put ws, col, r, "arrow_length", GetAny(arrow, "length")
      Put ws, col, r, "arrow_thickness", GetAny(arrow, "thickness")
      Put ws, col, r, "arrow_radius", GetAny(arrow, "radius")
      Put ws, col, r, "arrow_height", GetAny(arrow, "height")

      Dim baseV As Variant: baseV = GetAny(arrow, "base")
      Put ws, col, r, "arrow_base_x", ArrGet(baseV, 0)
      Put ws, col, r, "arrow_base_y", ArrGet(baseV, 1)
    End If
  End If
End Sub
  ...
  For r = DATA_START_ROW To lastRow
    ...
    If Len(relKind) > 0 Or Len(relVal) > 0 Or Not capObj Is Nothing Or Len(sense) > 0 Then
      ...
      If Len(relKind) > 0 Then
        If Len(relVal) = 0 Then Err.Raise 10001, "ReadLines", "lines row " & r & ": relation_kind があるなら relation_value も要る"
    ...
    If ea Is Nothing Or eb Is Nothing Then Err.Raise 10001, "ReadLines", "lines row " & r & ": end_a / end_b は必須（ref or coord）"
    Set app("end_a") = ea
    Set app("end_b") = eb
  ...
  Select Case prim
    Case "line"
      If IsBlank(GetCell(ws, col, r, "arrow_length")) Or IsBlank(GetCell(ws, col, r, "arrow_thickness")) Then
        Err.Raise 10001, "BuildLineArrow", "lines row " & r & ": arrow line は length+thickness 必須"
      End If
    ...
    Case "cone"
      If IsBlank(GetCell(ws, col, r, "arrow_radius")) Or IsBlank(GetCell(ws, col, r, "arrow_height")) Then
        Err.Raise 10001, "BuildLineArrow", "lines row " & r & ": arrow cone は radius+height 必須"
      End If
    ...
    Case "pyramid"
      If IsBlank(GetCell(ws, col, r, "arrow_base_x")) Or IsBlank(GetCell(ws, col, r, "arrow_base_y")) Or IsBlank(GetCell(ws, col, r, "arrow_height")) Then
        Err.Raise 10001, "BuildLineArrow", "lines row " & r & ": arrow pyramid は base_x/base_y + height 必須"
      End If
      Dim base As New Collection
...
Private Sub WriteLocalizedString(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal keyStr As String, ByVal keyJa As String, ByVal keyEn As String, ByVal v As Variant)
  ...
  If IsObject(v) Then
    ...
    If HasKey(d, "ja") Then Put ws, col, r, keyJa, d("ja")
    If HasKey(d, "en") Then Put ws, col, r, keyEn, d("en")

    ' 代表値
    If HasKey(d, "ja") Then
      Put ws, col, r, keyStr, d("ja")
    ElseIf HasKey(d, "en") Then
      Put ws, col, r, keyStr, d("en")
    End If

  Else
    Put ws, col, r, keyStr, v
  End If
End Sub
...
Private Function BuildLocalizedStringObj(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal keyStr As String, ByVal keyJa As String, ByVal keyEn As String) As Object
  ...
  If Len(jaV) = 0 And Len(enV) = 0 Then
    ...
    ' schema上は string を許すので string を返したいが、戻り値をObjectにしてるからここは Nothing 扱いにする
    ' -> 呼び出し側で sV を拾えないので、ここは {ja} を作る方が安全
    d("ja") = sV
...
Private Sub Put(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal key As String, ByVal v As Variant)
  Dim lk As String: lk = LCase$(key)
...
Private Function Vec3FromCols(ByVal ws As Worksheet, ByVal col As Object, ByVal r As Long, ByVal kx As String, ByVal ky As String, ByVal kz As String) As Object
  ...
  If IsBlank(xV) Or IsBlank(yV) Or IsBlank(zV) Then
    Err.Raise 10001, "Vec3FromCols", "row " & r & ": " & kx & "/" & ky & "/" & kz & " は3つセット"
  End If

  Dim v As New Collection
  v.Add CDbl(xV): v.Add CDbl(yV): v.Add CDbl(zV)

  ' 配列として返したいので Collection を Variant で返すために Object にする
  ' JsonConverter は Collection を JSON array として出す
  Set Vec3FromCols = v
End Function
...
Private Function ParseFramesCell(ByVal v As Variant) As Variant
  ...
  If IsEmpty(parsed) Then
    Err.Raise 10001, "ParseFramesCell", "frames は数値 or JSON配列"
  End If
