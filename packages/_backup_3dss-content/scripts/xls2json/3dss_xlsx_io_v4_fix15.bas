...
Private Sub ApplyLocalizedGroup(ByVal ws As Worksheet, ByVal cols As Object, ByVal r As Long, ByVal node As Object, ByVal baseKey As String, ByVal basePath As String, ByVal touched As Object)
    ' baseKey is the UI column prefix (e.g. name/caption). basePath points to the JSON field (e.g. signification.name).
    ' Policy:
    ' - Blank cell = do not delete; keep existing (roundtrip-friendly)
    ' - To delete: put "__UNSET__" in the target cell

    If Not cols.Exists(baseKey) Then Exit Sub

    Dim sBase As String: sBase = TrimWs(CStr(ws.Cells(r, cols(baseKey)).Value))
    Dim jaKey As String: jaKey = baseKey & "_ja"
    Dim enKey As String: enKey = baseKey & "_en"

    Dim sJa As String: sJa = ""
    Dim sEn As String: sEn = ""
    If cols.Exists(jaKey) Then sJa = TrimWs(CStr(ws.Cells(r, cols(jaKey)).Value))
    If cols.Exists(enKey) Then sEn = TrimWs(CStr(ws.Cells(r, cols(enKey)).Value))

    If Len(sBase) = 0 And Len(sJa) = 0 And Len(sEn) = 0 Then Exit Sub

    ' __UNSET__ at baseKey deletes the whole field
    ...
    ' If default/base is provided, prefer the scalar form (schema may allow string or localized object)
    If Len(sBase) > 0 Then
    ...
    ' Localized merge: do NOT replace the whole object; only update keys provided.
    Dim curV As Variant
    Call JsonGetByPathAny(node, basePath, curV)

    Dim o As Object
    If IsObject(curV) And TypeName(curV) = "Dictionary" Then
        Set o = curV
    Else
        Set o = CreateDict()
    ...
    Dim didChange As Boolean: didChange = False

    If Len(sJa) > 0 Then
        If UCase$(sJa) = "__UNSET__" Then
            If o.Exists("ja") Then o.Remove "ja"
            touched(jaKey) = "__UNSET__"
            didChange = True
        Else
            o("ja") = sJa
            touched(jaKey) = sJa
    ...
    If Len(sEn) > 0 Then
        If UCase$(sEn) = "__UNSET__" Then
            If o.Exists("en") Then o.Remove "en"
            touched(enKey) = "__UNSET__"
            didChange = True
        Else
            o("en") = sEn
            touched(enKey) = sEn
    ...
    ' If we only removed keys and it became empty, drop the whole object
    If didChange Then
        If o.Count = 0 Then JsonDeleteByPath node, basePath
    End If
End Sub
...
Private Sub ApplyRelationGroup(ByVal ws As Worksheet, ByVal cols As Object, ByVal r As Long, ByVal node As Object, ByVal touched As Object)
  ...
  Dim k As String: k = TrimWs(CStr(GetByHeader(ws, cols, r, "relation_kind")))
  Dim v As String: v = TrimWs(CStr(GetByHeader(ws, cols, r, "relation_value")))
...
Private Sub ApplyEndpointGroup(ByVal ws As Worksheet, ByVal cols As Object, ByVal r As Long, ByVal node As Object, ByVal whichEnd As String, ByVal touched As Object)
  Dim refS As String: refS = TrimWs(CStr(GetByHeader(ws, cols, r, whichEnd & "_ref")))
  Dim sx As String: sx = TrimWs(CStr(GetByHeader(ws, cols, r, whichEnd & "_x")))
  Dim sy As String: sy = TrimWs(CStr(GetByHeader(ws, cols, r, whichEnd & "_y")))
  Dim sz As String: sz = TrimWs(CStr(GetByHeader(ws, cols, r, whichEnd & "_z")))
...
Private Sub ClearDataRows(ByVal ws As Worksheet, ByVal clearCols As Long)
  ...
  Set searchRange = ws.Range(ws.Cells(FIRST_DATA_ROW, 1), ws.Cells(ws.Rows.Count, clearCols))
  ...
  ws.Range(ws.Cells(FIRST_DATA_ROW, 1), ws.Cells(lastCell.Row, clearCols)).ClearContents
End Sub
...
Private Sub JsonGetByPathAny(ByVal root As Object, ByVal path As String, ByRef outV As Variant)
  ' Get value at dotted path into Variant safely (Object vs scalar).
  Dim o As Object
  outV = Empty
...
Private Function TryParseJsonToVariant(ByVal s As String, ByRef outV As Variant) As Boolean
  On Error GoTo Fail
  Dim tmp As Variant
  Set tmp = JsonConverter.ParseJson(s)
  Set outV = tmp
...
Private Function CellToJsonValue(ByVal s As String) As Variant
  ...
  ElseIf Left$(t, 1) = "{" Or Left$(t, 1) = "[" Then
    Set CellToJsonValue = JsonConverter.ParseJson(t)
...
Private Sub CellToJsonValueAny(ByVal s As String, ByRef outV As Variant)
    ' Parse JSON-like text into Variant (Object/scalar) safely.
    ' Uses TrimWs so leading CR/LF/TAB/nbsp does not break detection.
    Dim t As String: t = TrimWs(s)
    If Len(t) = 0 Then
        outV = Empty
        Exit Sub
    ...
    If Left$(t, 1) = "{" Or Left$(t, 1) = "[" Then
        Dim tmp As Variant
        If TryParseJsonToVariant(t, tmp) Then
            If IsObject(tmp) Then
                Set outV = tmp
            Else
                outV = tmp
